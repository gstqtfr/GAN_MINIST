...
num_vms=`az vm list  -g ${resource_group} | jq '. | length'`
first_ip_addr=`az vm list-ip-addresses -g ${resource_group} | jq .[0].virtualMachine.network.publicIpAddresses[0].ipAddress | sed 's/"//g'`
...
for node_id in `seq 0 $((num_vms - 1))`; do
    private_ip_addr=`az vm list-ip-addresses  -g ${resource_group} | jq .[${node_id}].virtualMachine.network.privateIpAddresses[0] | sed 's/"//g'`
